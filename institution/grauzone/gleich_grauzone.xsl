...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="grauzone">
		<xsl:element name="catalog">
			<xsl:for-each select="object">
				...
				<xsl:if
					test="(objektart[text()='A(1) - Akten']) or
				...
				(objektart[text()='Z - Zeitschriften (Reihe)']) ">
					...
					<xsl:if test="objektart[text()='A(1) - Akten']">
						...
						<xsl:element name="record">
							<xsl:attribute name="id"><xsl:value-of
								select="objektnummer" /></xsl:attribute>
						...
						<xsl:element name="record">
							...
							<xsl:element name="dataset">
								...
								<xsl:if test="Einzeltitel[string-length() != 0]">
									...
									<listOfContents>
										<xsl:value-of
											select="normalize-space(replace(Einzeltitel,'\\\[W9\]\\','&lt;br&gt;'))" />
							...
							<xsl:if test="s._Artikel[string-length() != 0]">
								<functions>
									<hierarchyFields>
										...
										<hierarchy_top_id>
											<xsl:value-of select="normalize-space(objektnummer)" />
										...
										<hierarchy_sequence>
											<xsl:value-of select="normalize-space(substring(Sammeltitel,1,4))" />
										</hierarchy_sequence>
					...
					<xsl:if test="objektart[text()='BA - Bücher/Artikel']">
						...
						<xsl:element name="record">
							...
							<xsl:element name="institution">
								...
								<link>
									<xsl:text>http://www.ida-dachverband.de/einrichtungen/deutschland/grauzone/</xsl:text>
								</link>
								<geoLocation>
							...
							<xsl:element name="dataset">
								...
								<xsl:variable name="connect">
									<xsl:for-each select="//s._Artikel">
										...
										<xsl:if test="contains(.,$id)">
											...
											<xsl:variable name="erschienen"
												select="translate(../Jahr, translate(.,'0123456789', ''), '')" />
											<xsl:for-each
												select="document('../../anreicherung/year.xml')/root/child/year">
								...
								<!--<test> <xsl:value-of select="$connect"></xsl:value-of> </test> -->
								...
								<xsl:if
									test="substring(substring-after($connect,'editor:'),1,1)!=':'">
									<xsl:for-each
										select="tokenize(substring-before(substring-after($connect,'editor:'),':editor'), ';')">
										<xsl:if
											test="(not(contains(.,'o. A.'))) and not(contains(.,'u.a.'))">
											<editor>
								...
								<xsl:if
									test="substring(substring-after($connect,'series:'),1,1)!=':'">
									<xsl:for-each
										select="tokenize(substring-before(substring-after($connect,'series:'),':series'), ';')">
										<series>
											<xsl:value-of select="normalize-space(.)" />
										</series>
									</xsl:for-each>
								...
								<xsl:if
									...
									<displayPublishDate>
										<xsl:value-of
											select="normalize-space(substring-before(substring-after($connect,'displayPublishDate:'),':displayPublishDate'))" />
									</displayPublishDate>
								</xsl:if>

								<xsl:if
									test="substring(substring-after($connect,'publishDate:'),1,1)!=':'">
									<publishDate>
										<xsl:value-of
							...
							<xsl:for-each select="//s._Artikel[contains(.,$objektnummer)]">
								...
								<functions>
									<hierarchyFields>
										...
										<hierarchy_sequence>
											<xsl:value-of select="normalize-space(substring($title,1,4))" />
										</hierarchy_sequence>
					...
					<xsl:if test="objektart[text()='F - Fotos/DIAS']">
						...
						<xsl:element name="record">
							...
							<xsl:element name="dataset">
								...
								<xsl:if
									test="(Anlass_Ereignis[string-length() = 0]) and (Beschreibung[string-length() != 0])">
									<title>
							...
							<xsl:element name="dataset">
								...
								<xsl:choose>
									<xsl:when test="contains(Titel,':')">
										<title>
											<xsl:value-of
												select="normalize-space(replace(Titel,'\\\[W9\]\\',''))" />
										</title>
										<title_short>
											<xsl:value-of
												select="normalize-space(substring-before(replace(Titel,'\\\[W9\]\\',''),':'))" />
										</title_short>
										<title_sub>
											<xsl:value-of
												select="normalize-space(substring-after(replace(Titel,'\\\[W9\]\\',''),':'))" />
										</title_sub>
									</xsl:when>
									<xsl:otherwise>
										<title>
					...
					<xsl:if test="objektart[text()='Z - Zeitschriften (Reihe)']">
						...
						<xsl:element name="record">
							...
							<!--<xsl:variable name="connect"> <xsl:variable name="rel" select="normalize-space(translate(s._ST[1], 
								translate(.,'-0123456789', ''), ''))"/> <xsl:for-each select="//object[objektnummer=$rel]"> 
								<xsl:text> title:</xsl:text> <xsl:value-of select="normalize-space(replace(Sachtitel,'\\\[W2\]\\',''))" 
								/> <xsl:text>:title</xsl:text> <xsl:text> title_sub:</xsl:text> <xsl:value-of 
								select="normalize-space(Untertitel)"></xsl:value-of> <xsl:text>:title_sub</xsl:text> 
								<xsl:text> hrsg:</xsl:text> <xsl:value-of select="normalize-space(Hrsg.)"></xsl:value-of> 
								<xsl:text>:hrsg</xsl:text> <xsl:text> placeOfPublication:</xsl:text> <xsl:value-of 
								select="Ersch.-ort"></xsl:value-of> <xsl:text>:placeOfPublication</xsl:text> 
								<xsl:text> publisher:</xsl:text> <xsl:if test="not(contains(Verlage,'o.A.'))"></xsl:if> 
								<xsl:value-of select="Verlage"></xsl:value-of> <xsl:text>:publisher</xsl:text> 
							...
							<xsl:element name="dataset">
								...
								<title>
									<xsl:value-of
										select="normalize-space(replace(Sachtitel,'\\\[W2\]\\',''))" />
								...
								<!--<collectionHolding> <xsl:value-of select="Jg."></xsl:value-of> 
									<xsl:value-of select="J."></xsl:value-of> </collectionHolding> -->
							...
							<xsl:if test="vorrät._Hefte[string-length() != 0]">
								...
								<xsl:variable name="rel"
									select="translate(vorrät._Hefte[1], translate(.,'0123456789', ''), '')" />
					...
					<xsl:if test="objektart[text()='Z - Zeitschriften (Heft)']">
						...
						<xsl:element name="record">
							...
							<xsl:variable name="connect">
								<xsl:variable name="rel"
									select="normalize-space(translate(s._ST[1], translate(.,'-0123456789', ''), ''))" />
							...
							<xsl:element name="dataset">
								...
								<xsl:choose>
									...
									<xsl:when
										test="($connect[string-length() != 0]) and (substring(substring-after($connect,'title:'),1,1)!=':')">
										<title>
											<xsl:value-of
												select="substring-before(substring-after($connect,'title:'),':title')" />
										</title>
										<title_short>
											<xsl:value-of
												select="substring-before(substring-after($connect,'title:'),':title')" />
										...
										<xsl:if
											test="substring(substring-after($connect,'title_sub:'),1,1)!=':'">
											<title_sub>
												<xsl:value-of
													select="substring-before(substring-after($connect,'title_sub:'),':title_sub')" />
											</title_sub>
										</xsl:if>
									</xsl:when>
									<xsl:otherwise>
										...
										<xsl:if test="Beschreibung[string-length() != 0]">
											<title_sub>
												<xsl:value-of
													select="normalize-space(substring(Beschreibung,1,150))" />
												<xsl:text> [...]</xsl:text>
											</title_sub>
										</xsl:if>
									</xsl:otherwise>
								...
								<!--ort -->
								<xsl:if
									test="($connect[string-length() != 0]) and (substring(substring-after($connect,'placeOfPublication:'),1,1)!=':')">
									<placeOfPublication>
										<xsl:value-of
											select="normalize-space(substring-before(substring-after($connect,'placeOfPublication:'),':placeOfPublication'))" />
									</placeOfPublication>
								</xsl:if>

								<!--verlag -->
								<xsl:if
									test="($connect[string-length() != 0]) and (substring(substring-after($connect,'publisher:'),1,1)!=':')">
									<publisher>
										<xsl:value-of
											select="normalize-space(substring-before(substring-after($connect,'publisher:'),':publisher'))" />
									</publisher>
								</xsl:if>
					...
					<xsl:if test="objektart[text()='Z - Zeitschriften (Artikel)']">
						...
						<xsl:element name="record">
							...
							<xsl:element name="dataset">
								...
								<!--<test> <xsl:value-of select="$connect"></xsl:value-of> <xsl:value-of 
									select="$id"></xsl:value-of> </test> -->
								...
								<xsl:choose>
									...
									<xsl:when
										test="($connect[string-length() != 0]) and (substring(substring-after($connect,'sourceInfo:'),1,1)!=':')">
										<title>
											<xsl:value-of
								...
								<xsl:if
									...
									<sourceInfo>
										<xsl:value-of
											select="normalize-space(substring-before(substring-after($connect,'sourceInfo:'),':sourceInfo'))" />
									</sourceInfo>
								</xsl:if>
								...
								<!--shelfMark Signatur -->
								<xsl:if
									test="($connect[string-length() != 0]) and (substring(substring-after($connect,'shelfMark:'),1,1)!=':')">
									<shelfMark>
										<xsl:value-of
											select="normalize-space(substring-before(substring-after($connect,'shelfMark:'),':shelfMark'))" />
									</shelfMark>
								</xsl:if>
							...
							<xsl:for-each select="//s._ET[contains(.,$objektnummer)]">
								...
								<functions>
									<hierarchyFields>
										...
										<hierarchy_top_title>
											<xsl:value-of select="normalize-space(../Hefttitel)" />
										</hierarchy_top_title>
							...
							<xsl:element name="dataset">
								...
								<xsl:choose>
									<xsl:when test="Ersch-dat.[string-length() != 0]">
										...
										<publishDate>
											<xsl:value-of
												select="normalize-space(translate(Ersch-dat., translate(.,'-0123456789', ''), ''))" />
										</publishDate>
									</xsl:when>
									<xsl:when test="Veranst.-dat.[string-length() != 0]">
										...
										<publishDate>
											<xsl:value-of
												select="normalize-space(translate(Veranst.-dat., translate(.,'-0123456789', ''), ''))" />
										</publishDate>
									</xsl:when>
								</xsl:choose>
								...
								<!--description -->
								<xsl:if
									test="(Anlass_Ereignis[string-length() != 0]) or (Beschreibung[string-length() != 0])">
									<description>
										<xsl:if test="Anlass_Ereignis[string-length() != 0]">
											<xsl:value-of select="normalize-space(Anlass_Ereignis)" />
											<xsl:text> - </xsl:text>
										...
										<xsl:if test="Beschreibung[string-length() != 0]">
											<!--<xsl:value-of select="normalize-space(Beschreibung)" /> -->
											<xsl:value-of
												select="normalize-space(replace(Beschreibung,'\\\[W9\]\\',' '))" />
										</xsl:if>
									</description>
								</xsl:if>
	...
	<!--<xsl:template match="Orte"> <placeOfPublication> <xsl:value-of select="normalize-space(.)" 
		/> </placeOfPublication> </xsl:template> -->
	...
	<xsl:template match="Beschreibung">
		<description>
			...
			<xsl:value-of
				select="normalize-space(replace(replace(.,'\\\[W8\]\\',''),'\\\[W9\]\\',''))" />
		</description>
	</xsl:template>
	...
	<xsl:template match="Auflage">
		<edition>
			<xsl:value-of
				select="normalize-space(translate(., translate(.,'0123456789', ''), ''))" />
		</edition>
	</xsl:template>
			...
			<xsl:when test="../objektart[text()='Z - Zeitschriften (Heft)']">
				<xsl:if test="contains(.,'(')">
					<xsl:if test="matches(substring(substring-after(.,')'),1,2),'[0-9]')">
						<issue>
							<xsl:value-of
								select="normalize-space(substring(substring-after(.,')'),1,2))" />
						</issue>
					</xsl:if>
					<displayPublishDate>
	...
	<xsl:template match="Enthält">
		<description>
			<xsl:value-of
				select="normalize-space(replace(replace(.,'\\\[W2\]\\',''),'\\\[W9\]\\',''))" />
	...
	<xsl:template match="Anlass_Ereignis">
		...
		<!--<xsl:if test="../Beschreibung[string-length() != 0]"> <title_sub> <xsl:value-of 
			select="normalize-space(../Beschreibung)" /> </title_sub> </xsl:if> -->
	</xsl:template>
	...
	<xsl:template match="Hefttitel">
		<xsl:choose>
			<xsl:when test="contains(.,':')">
				...
				<title_short>
					<xsl:value-of
						select="normalize-space(substring-before(replace(.,'\\\[W9\]\\',''),':'))" />
				</title_short>
				<title_sub>
					<xsl:value-of
						select="normalize-space(substring-after(replace(.,'\\\[W9\]\\',''),':'))" />
				</title_sub>
			</xsl:when>
			<xsl:otherwise>
				<title>
				...
				<!--<title> <xsl:value-of select="normalize-space(.)" /> </title> <title_short> 
